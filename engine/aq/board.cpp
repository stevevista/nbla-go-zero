...
Board& Board::operator=(const Board& other) {
	...
	copy(other.board_history.begin(), other.board_history.end(), board_history.begin());
...
void Board::PlayLegal(int v) {
	...
	if (v == PASS) {
		...
		std::rotate(board_history.rbegin(), board_history.rbegin() + 1,
			board_history.rend());
		std::copy(&color[0], &color[0] + BVCNT, board_history[0].begin());
