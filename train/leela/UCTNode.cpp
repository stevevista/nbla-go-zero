...
bool UCTNode::create_children(std::atomic<int> & nodecount,
    ...
    auto legal_sum = 0.0f;
    for (auto idx=0; idx < FastBoard::BOARDSQ; idx++) {
        if (state.is_move_legal(to_move, idx)) {
            nodelist.emplace_back(Network::scored_node{raw_netlist.first[idx], idx});
            legal_sum += raw_netlist.first[idx];
        }
    }
    nodelist.emplace_back(Network::scored_node{raw_netlist.first[FastBoard::BOARDSQ], FastBoard::PASS});
    legal_sum += raw_netlist.first[FastBoard::BOARDSQ];
    ...
    for (const auto& node : nodelist) {
        m_children.emplace_back(
            std::make_unique<UCTNode>(node.second, node.first, net_eval)
        );
    }
