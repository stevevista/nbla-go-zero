...
vector<MoveData> GameArchive::next_batch(int count, bool fitsize, bool& rewinded, int min_steps) {
    ...
    while (out.size() < count) {
        ...
        if (min_steps > 0 && entries_[data_index].second < min_steps) {
            data_index++;
            continue;
        }
        ...
        extract_move(data_index++, planes, dist, result);
        out.push_back({planes, dist, result});
    }
...
void GameArchive::extract_move(const int index, PLANES& planes, PROBS& probs, int& result) {
    ...
    result = games_[ind.first].result;

    const int steps = ind.second;
    
    GoBoard::get_board_features(planes, game_rec.begin(), game_rec.begin() + steps);
    ...
    probs.resize(362);
    int move = game_rec[steps];
    std::fill(probs.begin(), probs.end(), 0);

    if (move < 0) probs[361] = 1;
    else probs[move] = 1;
}
