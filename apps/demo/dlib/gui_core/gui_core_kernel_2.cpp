...
// ----------------------------------------------------------------------------------------
        ...
        private:
            ...
            )
            {
                ...
                try
                {
                    ...
                    if (XInitThreads() == 0)
                    {
                        std::cerr  << "Unable to initialize threading support.";
                    ...
                    if (disp == 0)
                    {
                        ...
                        if (disp == 0)
                        {
                            std::cerr  << "Unable to connect to the X display.";
                            // signal that an error has occurred
                            window_table.get_mutex().lock();
                            status = failure_to_init;
                            et_signaler.broadcast();
    ...
    // ----------------------------------------------------------------------------------------
        ...
        {       
            ...
            {
                std::cerr  << "Exception thrown in event handler: " << e.what();
            }
            catch (...)
            {
                std::cerr  << "Unknown exception thrown in event handler.";
            }
        }
    ...
    // ----------------------------------------------------------------------------------------
        ...
        init_keyboard_mod_masks()
        {
            ...
            if ( alt_mask == 0 )
            {
                std::cerr  << "Search for Alt-key faild.";
                if ( meta_mask != 0 )
                    alt_mask = meta_mask;
                else
